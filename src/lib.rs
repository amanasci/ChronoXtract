...
fn chronoxtract(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(carma::carma_mcmc, m)?)?;
    m.add_function(wrap_pyfunction!(carma::simulate_carma, m)?)?;
    m.add_function(wrap_pyfunction!(carma::generate_carma_data, m)?)?;
    m.add_function(wrap_pyfunction!(carma::generate_stable_carma_parameters, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(carma::carma_model_selection, m)?)?;
    m.add_function(wrap_pyfunction!(carma::carma_power_spectrum, m)?)?;
    m.add_function(wrap_pyfunction!(carma::carma_autocovariance, m)?)?;
