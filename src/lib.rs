...
fn chronoxtract(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(carma::kalman::carma_loglikelihood, m)?)?;
    m.add_function(wrap_pyfunction!(carma::mle::carma_mle, m)?)?;
    m.add_function(wrap_pyfunction!(carma::mcmc::carma_mcmc, m)?)?;
    m.add_function(wrap_pyfunction!(carma::selection::carma_choose_order, m)?)?;
    m.add_function(wrap_pyfunction!(carma::predict::carma_predict, m)?)?;
    
    // CARMA types
    m.add_class::<carma::types::CarmaParams>()?;
    m.add_class::<carma::types::McmcParams>()?;
    m.add_class::<carma::types::CarmaMLEResult>()?;
    m.add_class::<carma::types::CarmaMCMCResult>()?;
